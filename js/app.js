...
(function () {
    ...
    // Debug-панель включается при ?debug=1 в URL (для диагностики)
    ...
    Promise.all([minDelay, trackOpen()])
        .then(function () {
            // В debug-режиме НЕ редиректим (чтобы видеть статус)
            if (DEBUG_MODE) {
                showDebug('REDIRECT SKIPPED (debug mode). Target: ' + targetUrl);
                return;
            }
